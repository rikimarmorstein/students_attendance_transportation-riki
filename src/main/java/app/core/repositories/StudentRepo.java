...
//    List<Student> findAllBySchoolIdAndNumBusIdAndIsTravelTrue(int schoolId, int numBus );
    List<Student> findAllBySchoolIdAndNumBusAndIsTravelTrue(int schoolId, int numBus );
   ...
   //צריך למחוק את זה אבל לא מחקתי כי המתודות משתמשות בזה, אז צריך קודם לשנות אותן ואז להשתמש במתודה מעל ולא בזאתי
    ...
    @Modifying
    @Query(value ="update `students` set is_travel = false , cause=?  where id =?;", nativeQuery = true)
    void isNotTravel(Student.Cause c, int id);

    @Modifying
    @Query(value ="update `students` set is_travel = true , cause=null  where id =?;", nativeQuery = true)
//    void Travel(int id);
